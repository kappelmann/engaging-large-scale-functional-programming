...
\begin{frame}
\only<1>{\centerline{\alert{\huge{It is hopeless!}}}}
\only<2->{\centerline{\huge{\sout{It is hopeless!}}}}

\vspace{\baselineskip}
\visible<2->{
\only<2->{\centerline{\alert{\huge{There is hope!}}}}
\begin{itemize}
\visible<3->{
\item We managed to cope with all these challenges}
\visible<4->{
\item We share our insights and tools for other educators}
\end{itemize}
...
\item Awards for top 30 students (conference tickets, Haskell keychains, prizes by industry partners,\dots)
\item Works extremely well to motivate talented students\dots
\item[] but it is very time-consuming.
\end{itemize}
% \begin{onlyenv}<2>
...
\only<6->{
\visible<6->{
\item Great success: 105 spots but 120 registrations}
\visible<7->{
